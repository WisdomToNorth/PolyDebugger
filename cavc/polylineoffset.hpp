                   ...
                   bool connectionArcsAreCCW, Polyline<Real> &result)
{
    ...
    auto processIntersect = [&](Real t, Vector2<Real> const &intersect)
    {
        ...
        if (trueSegIntersect && trueArcIntersect)
        {
            ...
            {
                addOrReplaceIfSamePos(result,
                                      PlineVertex<Real>(intersect, std::tan(theta / Real(4))));
            }
            else
            {
                  ...
                  bool connectionArcsAreCCW, Polyline<Real> &result)
{
    ...
    switch (intrResult.intrType)
    {
    case Circle2Circle2IntrType::NoIntersect: connectUsingArc(); break;
    case Circle2Circle2IntrType::OneIntersect: processIntersect(intrResult.point1); break;
    ...
    for (std::size_t sIndex : queryResults)
    {
        ...
        if (v1.bulgeIsZero())
        {
            ...
            else if (intrResult.numIntersects == 1)
            {
                ...
                {
                    output.emplace_back(sIndex,
                                        pointFromParametric(v1.pos(), v2.pos(), intrResult.t0));
                }
            }
            else
            {
                ...
                {
                    output.emplace_back(sIndex,
                                        pointFromParametric(v1.pos(), v2.pos(), intrResult.t1));
                }
            }
        }
        else
        {
            ...
            switch (intrResult.intrType)
            {
            case Circle2Circle2IntrType::NoIntersect: break;
            case Circle2Circle2IntrType::OneIntersect:
                if (validArcSegIntersect(arc.center, v1.pos(), v2.pos(), v1.bulge(),
                                         intrResult.point1))
                {
            ...
            case Circle2Circle2IntrType::TwoIntersects:
                ...
                break;
            case Circle2Circle2IntrType::Coincident: break;
            }
        }
    }
}
...
Polyline<Real> createRawOffsetPline(Polyline<Real> const &pline, Real offset)
{
    ...
    // detect single collapsed arc segment (this may be removed in the future if invalid segments
    // are tracked in join functions to be pruned at slice creation)
    ...
    if (pline.isClosed() && result.size() > 1)
    {
        ...
        // must do final singularity prune between first and second vertex after joining curves (n,
        // 0) and (0, 1)
        if (result.size() > 1)
        {
                                                         ...
                                                         Real offset)
{
    ...
    if (selfIntersects.size() == 0)
    {
        if (!pointValidForOffset(originalPline, offset, origPlineSpatialIndex,
                                 rawOffsetPline[0].pos(), queryStack))
        {
            return result;
        }
                               ...
                               Polyline<Real> const &dualRawOffsetPline, Real offset)
{
    ...
    // using map rather than unordered map since we want to construct the slices in vertex index
    ...
    {
        // find intersects between circles generated at original open polyline end points and raw
        // offset polyline
    ...
    if (!originalPline.isClosed())
    {
        ...
        while (true)
        {
            ...
            if (iter == intersectsLookup.end())
            {
                ...
                if (index != 0
                    && intersectsOrigPline(firstSlice.lastVertex(), rawOffsetPline[index]))
                {
                    break;
                }
        ...
        while (true)
        {
            ...
            std::sort(queryResults.begin(), queryResults.end(),
                      [&](std::size_t index1, std::size_t index2)
                      {
                          ...
                          {
                              // index distances are equal, compare on position being equal to
                              // initial start (testing index1 < index2, we want the longest closed
                              // loop possible)
            ...
            if (queryResults.size() == 0)
            {
                ...
                {
                    if (closedPolyline
                        && fuzzyEqual(currPline[0].pos(), currPline.lastVertex().pos(),
                                      utils::realPrecision<Real>()))
                    {
