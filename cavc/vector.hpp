...
public:
    ...
    Vector(std::initializer_list<Real> values)
    {
        ...
        {
            std::copy(values.begin(), values.end(), m_data.begin());
            std::fill(m_data.begin() + static_cast<std::ptrdiff_t>(values.size()), m_data.end(),
                      Real(0));
        }
        else
        {
    ...
    inline Real const &operator[](std::size_t i) const
    {
        return m_data[i];
    }
    ...
    void makeOnes()
    {
        std::fill(m_data.begin(), m_data.end(), Real(1));
    }
...
template <typename Real, std::size_t N>
Vector<Real, N> operator+(Vector<Real, N> const &v)
{
    return v;
}
...
template <std::size_t N, typename Real>
Real length(Vector<Real, N> const &v)
{
    return std::sqrt(dot(v, v));
}

template <std::size_t N, typename Real>
Real normalize(Vector<Real, N> &v)
{
