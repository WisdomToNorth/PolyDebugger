...
struct ProcessForCombineResult
{
    ...
    bool plineOpposingDirections() const
    {
        return pline1IsCW != pline2IsCW;
    }
...
{
    CAVC_ASSERT(pline1.isClosed() && pline2.isClosed(),
                "combining only works with closed polylines");
        ...
        if (intrsList.size() != 1)
        {
            ...
            for (std::size_t i = 1; i < intrsList.size(); ++i)
            {
                SplitResult<Real> split =
                    splitAtPoint(prevVertex, firstSegEndVertex, intrsList[i].pos);
                                    ...
                                    bool setOpposingOrientation)
{
    ...
    if (setOpposingOrientation != combineInfo.plineOpposingDirections())
    {
        ...
        for (std::size_t i = result.startOfPlineBSlicesIdx;
             i < result.startOfPlineACoincidentSlicesIdx; ++i)
        {
            invertDirection(slicesRemaining[i]);
        }

        for (std::size_t i = result.startOfPlineBCoincidentSlicesIdx; i < slicesRemaining.size();
             ++i)
        {
            invertDirection(slicesRemaining[i]);
        }
                                       ...
                                       Real joinThreshold = utils::sliceJoinThreshold<Real>())
{
    ...
    for (std::size_t i = 0; i < slices.size(); ++i)
    {
        ...
        while (true)
        {
            ...
            currPline.vertexes().insert(currPline.vertexes().end(),
                                        connectedSlice.vertexes().begin(),
                                        connectedSlice.vertexes().end());
            visitedSliceIndexes[connectedSliceIndex] = true;

            // else continue stitching slices to current polyline, using last stitched index to find
            // next
                                     ...
                                     PlineCombineMode combineMode)
{
        ...
        return [=](std::size_t currSliceIndex, std::vector<std::size_t> const &available)
        {
            if (currSliceIndex >= startOfPlineACoincidentSlicesIdx)
            {
                ...
                if (currSliceIndex < startOfPlineBCoincidentSlicesIdx)
                {
                    ...
                    auto idx = std::find_if(available.begin(), available.end(),
                                            [&](auto i) {
                                                return i >= startOfPlineBSlicesIdx
                                                       && i < startOfPlineACoincidentSlicesIdx;
                                            });
                    ...
                    // no noncoincident slices available and stitching coincident to coincident is
                    // never allowed (discarded)
                    return std::numeric_limits<std::size_t>::max();
                }
    ...
    auto performUnion = [&]
    {
        ...
        else
        {
            ...
            auto stitchSelector = createUnionAndIntersectStitchSelector(
                collectedSlices.startOfPlineACoincidentSlicesIdx);

            std::vector<Polyline<Real>> remaining = stitchOrderedSlicesIntoClosedPolylines(
                collectedSlices.slicesRemaining, stitchSelector);
    ...
    auto performExclude = [&]
    {
        ...
        else
        {
            ...
            auto collectedSlices = collectSlices(
                plineA, plineB, combineInfo, [&](auto pt) { return !pointInB(pt); }, pointInA,
                true);
            ...
            result.remaining = stitchOrderedSlicesIntoClosedPolylines(
                collectedSlices.slicesRemaining, stitchSelector);
        }
    };
    ...
    auto performIntersect = [&]
    {
        ...
        else
        {
            ...
            auto collectedSlices =
                collectSlices(plineA, plineB, combineInfo, pointInB, pointInA, false);
        ...
        else
        {
            // collect A excluding B results
            {
                ...
                auto stitchSelector = createExcludeAndXORStitchSelector(
                    collectedSlices.startOfPlineBSlicesIdx,
                    collectedSlices.startOfPlineACoincidentSlicesIdx,
                    collectedSlices.startOfPlineBCoincidentSlicesIdx);
                result.remaining = stitchOrderedSlicesIntoClosedPolylines(
                    collectedSlices.slicesRemaining, stitchSelector);
            ...
            // collect B excluding A results
            {
                ...
                auto collectedSlices = collectSlices(
                    plineA, plineB, combineInfo, pointInB, [&](auto pt) { return !pointInA(pt); },
                    true);
                ...
                auto stitchSelector = createExcludeAndXORStitchSelector(
                    ...
                    collectedSlices.startOfPlineACoincidentSlicesIdx,
                    collectedSlices.startOfPlineBCoincidentSlicesIdx);
                auto stitchedResults = stitchOrderedSlicesIntoClosedPolylines(
                    collectedSlices.slicesRemaining, stitchSelector);
    ...
    switch (combineMode)
    {
    case PlineCombineMode::Union: performUnion(); break;
    case PlineCombineMode::Exclude: performExclude(); break;
    case PlineCombineMode::Intersect: performIntersect(); break;
    case PlineCombineMode::XOR: performXOR(); break;
    }
