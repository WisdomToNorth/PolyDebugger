...
private:
    ...
    {
        return i < m_inputSet->ccwLoops.size() ?
                   m_inputSet->ccwLoops[i] :
    ...
    std::size_t totalOffsetLoopsCount()
    {
        return m_ccwOffsetLoops.size() + m_cwOffsetLoops.size();
    }
...
template <typename Real>
void ParallelOffsetIslands<Real>::createOffsetLoops(const OffsetLoopSet<Real> &input, Real absDelta)
{
...
template <typename Real>
void ParallelOffsetIslands<Real>::createOffsetLoopsIndex()
{
...
template <typename Real>
void ParallelOffsetIslands<Real>::createSlicePoints()
{
    ...
    for (std::size_t i = 0; i < totalOffsetCount; ++i)
    {
        ...
        for (std::size_t j : m_queryResults)
        {
            ...
            if (intrsResults.hasIntersects())
            {
                ...
                {
                    auto coinSliceResult = sortAndjoinCoincidentSlices(
                        intrsResults.coincidentIntersects, loop1.polyline, loop2.polyline);
                                                     ...
                                                     Real absDelta)
{
    ...
    for (std::size_t i = 0; i < inputTotalCount; ++i)
    {
        ...
        if (!internal::pointValidForOffset(parentLoop.polyline, absDelta, parentLoop.spatialIndex,
                                           pt, m_queryStack))
        {
            return false;
        }
                                                       ...
                                                       std::vector<DissectedSlice> &result)
{
    ...
    for (auto const &kvp : m_loopDissectionPoints)
    {
        ...
        {
            auto sMidpoint = segMidpoint(currSlice.pline[currSlice.pline.size() - 2],
                                         currSlice.pline.lastVertex());
            if (!pointOnOffsetValid(parentIndex, sMidpoint, absDelta))
            {
                // skip slice
                continue;
