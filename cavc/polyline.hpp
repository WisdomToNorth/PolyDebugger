...
public:
    ...
    void addVertex(Real x, Real y, Real bulge)
    {
        m_vertexes.emplace_back(x, y, bulge);
    }
    ...
    std::vector<PVertex> const &vertexes() const
    {
        return m_vertexes;
    }
    ...
    void visitSegIndices(VisitorF &&visitor) const
    {
...
template <typename Real>
void scalePolyline(Polyline<Real> &pline, Real scaleFactor)
{
...
/// box is returned.
template <typename Real>
AABB<Real> getExtents(Polyline<Real> const &pline)
{
    if (pline.size() == 0)
    {
...
template <typename Real>
Real getArea(Polyline<Real> const &pline)
{
    ...
    // clockwise oriented polygons and positive value for counter clockwise oriented polygons. The
    // area of each circular segment defined by arcs is then added if it is a counter clockwise arc
    // or subtracted if it is a clockwise arc. The area of the circular segments are computed by
    // finding the area of the arc sector minus the area of the triangle defined by the chord and
    // center of circle. See https://en.wikipedia.org/wiki/Circular_segment
...
public:
    ...
    /// The closest point
    Vector2<Real> const &point() const
    {
        return m_point;
    }
...
template <typename Real>
void invertDirection(Polyline<Real> &pline)
{
    if (pline.size() < 2)
    {
        return;
...
template <typename Real>
Real getPathLength(Polyline<Real> const &pline)
{
    if (pline.size() < 2)
    {
        return Real(0);
...
int getWindingNumber(Polyline<Real> const &pline, Vector2<Real> const &point)
{
    ...
    auto arcVisitor = [&](const auto &v1, const auto &v2)
    {
        ...
        // count it as left going one direction and not left going the other (similar to using <=
        // for end points)
        bool pointIsLeft =
