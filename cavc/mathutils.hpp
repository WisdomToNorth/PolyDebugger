...
template <typename Real>
constexpr Real sliceJoinThreshold()
{
    return Real(1e-4);
}
...
template <typename Real>
constexpr Real offsetThreshold()
{
    return Real(1e-4);
}
...
template <typename Real>
bool fuzzyEqual(Real x, Real y, Real epsilon = realThreshold<Real>())
{
...
template <typename Real>
Real normalizeRadians(Real angle)
{
...
template <typename Real>
Real deltaAngle(Real angle1, Real angle2)
{
...
template <typename T>
std::size_t nextWrappingIndex(std::size_t index, const T &container)
{
...
template <typename T>
std::size_t prevWrappingIndex(std::size_t index, const T &container)
{
    if (index == 0)
    {
