...
public:
    ...
    PlineVertex(Vector2<Real> position, Real bulge) : PlineVertex(position.x(), position.y(), bulge)
    {
    }
...
AABB<Real> createFastApproxBoundingBox(PlineVertex<Real> const &v1, PlineVertex<Real> const &v2)
{
    ...
    // For arcs we don't compute the actual extents which is slower, instead we create an
    // approximate bounding box from the rectangle formed by extending the chord by the sagitta,
    // NOTE: this approximate bounding box is always equal to or bigger than the true bounding box
    Real b = v1.bulge();
...
template <typename Real>
Real segLength(PlineVertex<Real> const &v1, PlineVertex<Real> const &v2)
{
                                        ...
                                        PlineVertex<Real> const &u1, PlineVertex<Real> const &u2)
{
                                        ...
                                        PlineVertex<Real> const &a1, PlineVertex<Real> const &a2)
    {
        ...
        auto pointInSweep = [&](Real t)
        {
            ...
            bool withinSweep =
                pointWithinArcSweepAngle(arc.center, a1.pos(), a2.pos(), a1.bulge(), p);
    ...
    if (vIsLine && uIsLine)
    {
        ...
        {
        case LineSeg2LineSeg2IntrType::None: result.intrType = PlineSegIntrType::NoIntersect; break;
        ...
        case LineSeg2LineSeg2IntrType::Coincident:
            ...
            // build points from parametric parameters (using second segment as defined by the
            // function)
    ...
    else
    {
        ...
        auto startAndSweepAngle =
            [](Vector2<Real> const &sp, Vector2<Real> const &center, Real bulge)
        {
        ...
        case Circle2Circle2IntrType::Coincident:
            ...
            else
            {
                ...
                const bool arc2EndsInArc1Sweep = utils::angleIsWithinSweep(
                    arc1StartAndSweep.first, arc1StartAndSweep.second, arc2End);
