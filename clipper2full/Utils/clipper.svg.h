...
public:
    PathInfo(const PathsD &paths, bool is_open, FillRule fillrule, unsigned brush_clr,
             unsigned pen_clr, double pen_width, bool show_coords) :
        paths_(paths),
        is_open_path(is_open), fillrule_(fillrule), brush_color_(brush_clr), pen_color_(pen_clr),
...
class SvgWriter
{
    ...
    public:
        ...
        TextInfo(const std::string &txt, const std::string &fnt_name, unsigned color,
            ...
            text(txt),
            font_name(fnt_name), font_color(color), font_weight(weight), font_size(size),
            x(coord_x), y(coord_y){};
        friend class SvgWriter;
    };
...
public:
    ...
    void AddPath(const Path64 &path, bool is_open, FillRule fillrule, unsigned brush_color,
                 unsigned pen_color, double pen_width, bool show_coords);
    void AddPath(const PathD &path, bool is_open, FillRule fillrule, unsigned brush_color,
                 unsigned pen_color, double pen_width, bool show_coords);
    void AddPaths(const PathsD &paths, bool is_open, FillRule fillrule, unsigned brush_color,
                  unsigned pen_color, double pen_width, bool show_coords);
    void AddPaths(const Paths64 &paths, bool is_open, FillRule fillrule, unsigned brush_color,
                  unsigned pen_color, double pen_width, bool show_coords);
...
private:
    PathInfoList path_infos;
    bool LoadPath(std::string::const_iterator &p, const std::string::const_iterator &pe);
