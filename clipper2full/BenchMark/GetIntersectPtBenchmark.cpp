...
static int64_t divide(Int128 dividend, Int128 divisor)
{
    ...
    {
        divisor.hi <<= 1;
        if ((int64_t)divisor.lo < 0)
            divisor.hi++;
        divisor.lo <<= 1;

        cntr.hi <<= 1;
        if ((int64_t)cntr.lo < 0)
            cntr.hi++;
        cntr.lo <<= 1;
    }
    divisor.lo >>= 1;
...
typedef std::function<bool(const Point64 &, const Point64 &, const Point64 &, const Point64 &,
                           Point64 &)>
    GipFunction;
...
static bool GIP_Current(const Point64 &ln1a, const Point64 &ln1b, const Point64 &ln2a,
                        const Point64 &ln2b, Point64 &ip)
{
...
static bool GIP_Func_F(const Point64 &ln1a, const Point64 &ln1b, const Point64 &ln2a,
                       const Point64 &ln2b, Point64 &ip)
{
...
static bool GIP_F_Mod(const Point64 &ln1a, const Point64 &ln1b, const Point64 &ln2a,
                      const Point64 &ln2b, Point64 &ip)
{
    double ln1dy = (double)(ln1b.y - ln1a.y);
    double ln1dx = (double)(ln1a.x - ln1b.x);
...
static bool GIP_128(const Point64 &ln1a, const Point64 &ln1b, const Point64 &ln2a,
                    const Point64 &ln2b, Point64 &ip)
{
...
public:
    ...
    TestRecord(const Point64 &intersect_pt, const Point64 &p1, const Point64 &p2, const Point64 &p3,
               const Point64 &p4) :
        actual(intersect_pt),
...
int main(int argc, char **argv)
{
    ...
    for (int current_pow10 = 12; current_pow10 <= 18; ++current_pow10)
    {
        ...
        for (int64_t i = 0; i < 100000; ++i)
        {
            ...
            if (std::abs(GetSineFrom3Points(ip1, actual, ip3)) < sine_min_angle)
                continue;
        ...
        if (first_pass)
        {
            ...
            first_pass = false;
            std::cout << std::endl
                      << SetConsoleTextColor(green_bold)
                      << "Benchmark GetIntersectPoint performance ... "
            ...
            std::cout
                << std::endl
                << std::endl
                << SetConsoleTextColor(green_bold) << "Compare function accuracy ..."
                << SetConsoleTextColor(reset) << std::endl
                << "and show how it deteriorates when using very large coordinate ranges."
                << std::endl
                << "Distance error is the distance between the calculated and actual intersection points."
                << std::endl
                << "(The largest errors will occur whenever the segments are close to collinear.)"
                << std::endl;
        }
        else
        {
        ...
        std::cout << std::endl
                  << SetConsoleTextColor(cyan_bold) << "Coordinate ranges between  +/-10^"
                  << current_pow10 << SetConsoleTextColor(reset) << std::endl;
        ...
        for (int i = 0; i < number_of_test_functions; ++i)
        {
            ...
            std::cout << std::fixed << GetGipFuncName(i)
                      ...
                      << "; largest dist. = " << std::setprecision(0) << worst_dists[i]
                      << std::endl;
        }
        tests.clear();
    }
