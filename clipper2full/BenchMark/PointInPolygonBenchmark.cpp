...
inline PointInPolygonResult PIP2(const Point64 &pt, const Path64 &polygon)
{
    ...
    if (prev->y == pt.y)
    {
        if (pt == *prev)
            return PointInPolygonResult::IsOn;
        if ((curr->y == pt.y) && ((curr->x == pt.x) || ((pt.x > prev->x) == (pt.x < curr->x))))
            return PointInPolygonResult::IsOn;
        Path64::const_reverse_iterator pr = polygon.crbegin() + 1;
        while (pr != polygon.crend() && pr->y == pt.y)
            ++pr;
        is_above = pr == polygon.crend() || pr->y < pt.y;
    }
    else
        is_above = prev->y < pt.y;
...
static PointInPolygonResult PIP3(const Point64 &pt, const Path64 &path)
{
    ...
    for (; itCurr != path.cend(); ++itCurr)
    {
        ...
        else if ((y1 == 0) && (y2 == 0) && (((x2 <= 0) && (x1 >= 0)) || ((x1 <= 0) && (x2 >= 0))))
            return PointInPolygonResult::IsOn;
        itPrev = itCurr;
    }
    if (k % 2)
                                  ...
                                  PipFunction pip_func, PointInPolygonResult expected)
{
    ...
    for (Point64 poi : pts_of_int)
    {
        ...
        for (const Path64 &path : paths)
            if (pip_func(poi, path) == PointInPolygonResult::IsInside)
                ++inside_cnt;
        switch (expected)
        {
        case PointInPolygonResult::IsInside:
            if (inside_cnt != 1)
                error_points.push_back(poi);
            break;
    ...
    if (error_points.size())
    {
        ...
        for (size_t i = 0; i < high_error; ++i)
            std::cout << "(" << error_points[i] << "), ";
        std::cout << "(" << error_points[high_error] << ")." << SetConsoleTextColor(reset)
                  << std::endl;
    }
    else
...
static void DoErrorTest1(int index)
{
    ...
    std::cout << SetConsoleTextColor(green_bold) << "Testing PIP" << index + 1
              << "/outside:" << SetConsoleTextColor(reset);
    DoErrorTest1_internal(points_of_interest_outside, paths, pip_func,
                          PointInPolygonResult::IsOutside);

    std::cout << SetConsoleTextColor(green_bold) << "Testing PIP" << index + 1
              << "/inside :" << SetConsoleTextColor(reset);
    DoErrorTest1_internal(points_of_interest_inside, paths, pip_func,
...
static void DoErrorTest2(int index)
{
    ...
    std::cout << SetConsoleTextColor(green_bold) << "Testing PIP" << index + 1
              << SetConsoleTextColor(reset) << ":";
    for (size_t i = 0; i < paths.size(); ++i)
        if (pip_func(mp, paths[i]) != pipResults[0][i])
            errors.push_back(i);
    if (errors.size())
    {
        ...
        std::cout << errors[high_error] << "." << SetConsoleTextColor(reset) << std::endl;
    }
    else
        std::cout << " No errors found." << std::endl;
}
...
{
    std::cout << SetConsoleTextColor(cyan_bold) << "Simple error checks ..."
    ...
    std::cout << std::endl
              << SetConsoleTextColor(yellow_bold)
              << "Tests for errors #1:" << SetConsoleTextColor(reset) << std::endl
              << "(Reusing 'TestPolytreeHoles' tests)" << std::endl
              << std::endl;
    ...
    std::cout << std::endl
              ...
              << "(Testing with 'unusual' polygons)" << std::endl
              << std::endl;
    ...
    std::cout << std::endl
              << SetConsoleTextColor(cyan_bold) << "Benchmarking ..." << SetConsoleTextColor(reset)
              << std::endl;
    std::cout << "Note: function performance varies depending on the proportion of edges"
              << std::endl
              << "that intersect with an imaginary horizontal line passing through the" << std::endl
              << "point of interest." << std::endl
              << std::endl;
    ...
    std::cout << std::endl
              << SetConsoleTextColor(yellow_bold) << "Benchmarks 1:" << SetConsoleTextColor(reset)
              << std::endl;

    paths.clear();
    for (int i = 0; i < 5; ++i)
        paths.push_back(Ellipse(mp, width / 2.0, height / 2.0, count));
    std::cout << "A single elliptical path (" << width << " x " << height << ")" << std::endl
    ...
    for (size_t i = 0; i < 3; ++i)
        pipResults[i].resize(paths.size());
    ...
    std::cout << std::endl
              << std::endl
              << SetConsoleTextColor(yellow_bold) << "Benchmarks 2:" << SetConsoleTextColor(reset)
              << std::endl;

    std::cout << "A random self-intersecting polygon (" << width << " x " << height << ")"
              << std::endl
              << "Edge count =  " << count << ". " << std::endl
              << "Point (" << mp << ")" << std::endl
              << std::endl;
    ...
    {
        if ((pipResults[0][i] == pipResults[1][i]) && (pipResults[0][i] == pipResults[2][i]))
            continue;
