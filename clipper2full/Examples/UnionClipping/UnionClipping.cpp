...
void DoSquares()
{
    ...
    // SaveTest("squares.txt", false, &subjects, nullptr, nullptr, 0, 0, ClipType::Union,
    // FillRule::NonZero);
...
void DoCircles()
{
    ...
    for (int j = 0; j < 550; ++j)
    {
        subjects.push_back(TranslatePath(shape, rand() % (w - 50), rand() % (h - 50)));
    }
...
void DoTriangles()
{
    ...
    for (int i = 0; i < h / size / 2; ++i)
    {
        ...
        {
            if (rand() % 5 != 1)
                subjects.push_back(tri1);
            if (rand() % 5 != 1)
                subjects.push_back(tri2);
