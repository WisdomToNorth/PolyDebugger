...
void DoLoopThruPolygons(int start, int end)
{
    ...
    {
        if (!LoadTestNum(ifs, test_number, subject, subject_open, clip, stored_area, stored_count,
                         ct, fr))
            break;
        Clipper64 c64;
        ...
        c64.AddClip(clip);
        if (!c64.Execute(ct, fr, solution, solution_open))
            return;
        ...
        if (do_all)
        {
            ...
            double area_diff =
                (double)stored_area <= 0 ? 0 : std::fabs((area / (double)stored_area) - 1.0);
            int count = (int)(solution.size());
            double count_diff =
                stored_count <= 0 ? 0 : std::abs(count - stored_count) / (double)stored_count;
            if (count_diff > 0.02 || (area_diff > 0.1))
            {
                ...
                if (count_diff > 0.02)
                    std::cout << test_number << ": counts " << stored_count << " vs " << count
                              << std::endl;
                if (area_diff > 0.1)
                    std::cout << test_number << ": areas  " << stored_area << " vs " << area
                              << std::endl;
            }
            continue;
        }
