...
Paths64 GenerateRandomPaths(std::default_random_engine &rng, int path_count, int edge_count)
{
    ...
    for (int path = 0; path < path_count; ++path)
    {
        ...
        for (int point = 0; point < path_length; ++point)
        {
            ...
            {
                result_path.emplace_back(center_x + first_point_coordinate_x(rng),
                                         center_y + first_point_coordinate_y(rng));
            }
            else
            {
                const auto &previous_point = result_path.back();
                result_path.emplace_back(previous_point.x + orthogonal_dist_next_point(rng),
...
void RandomTest(int path_count)
{
    ...
    ClipType cliptype = static_cast<Clipper2Lib::ClipType>(GenerateRandomNumber(rng, 0, 3) + 1);
    FillRule fillrule = static_cast<Clipper2Lib::FillRule>(GenerateRandomNumber(rng, 0, 1));
    ...
    subj_open = GenerateRandomPaths(rng, GenerateRandomNumber(rng, 0, path_count),
                                    GenerateRandomNumber(rng, 3, max_edges));
    clip = GenerateRandomPaths(rng, GenerateRandomNumber(rng, 1, path_count),
                               GenerateRandomNumber(rng, 3, max_edges));
    ...
    c64.AddClip(clip);
    if (!c64.Execute(cliptype, fillrule, sol, sol_open))
        return;
    ...
    SvgAddCaption(svg, ClipTypeToString(cliptype) + ", " + FillRuleToString(fillrule), 20, 20);
