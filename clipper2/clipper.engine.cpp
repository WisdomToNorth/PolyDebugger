...
inline bool ClipperBase::IsContributingOpen(const Active &e) const
{
    ...
    case FillRule::Negative:
        ...
        is_in_subj = e.wind_cnt < 0;
        break;
    default: is_in_clip = e.wind_cnt2 != 0; is_in_subj = e.wind_cnt != 0;
    }
...
void ClipperBase::CleanCollinear(OutRec *outrec)
{
    ...
    for (;;)
    {
        ...
        if ((CrossProduct(op2->prev->pt, op2->pt, op2->next->pt) == 0)
            && (op2->pt == op2->prev->pt || op2->pt == op2->next->pt || !preserve_collinear_
    ...
    else
    {
        ...
        else if (old_e1_windcnt == 1 && old_e2_windcnt == 1)
        {
            ...
            case ClipType::Difference:
                ...
                break;
            case ClipType::Xor: resultOp = AddLocalMinPoly(e1, e2, pt, false); break;
            default:
                if (e1Wc2 > 0 && e2Wc2 > 0)
                    resultOp = AddLocalMinPoly(e1, e2, pt, false);
                break;
    ...
    for (; hs1 != hs_end1; ++hs1)
    {
        for (hs2 = hs1 + 1; hs2 != hs_end; ++hs2)
        {
            ...
            if (hs1->left_to_right)
            {
                ...
                HorzJoin join =
                    HorzJoin(DuplicateOp(hs1->left_op, true), DuplicateOp(hs2->left_op, false));
            ...
            else
            {
                ...
                HorzJoin join =
                    HorzJoin(DuplicateOp(hs2->left_op, true), DuplicateOp(hs1->left_op, false));
                horz_join_list_.push_back(join);
            }
        }
    }
...
bool ClipperBase::ResetHorzDirection(const Active &horz, const Vertex *max_vertex,
