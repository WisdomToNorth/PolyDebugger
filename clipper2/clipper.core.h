...
static void DoError([[maybe_unused]] int error_code)
{
    ...
    switch (error_code)
    {
    case precision_error_i: throw Clipper2Exception(precision_error);
    case scale_error_i: throw Clipper2Exception(scale_error);
    case non_pair_error_i: throw Clipper2Exception(non_pair_error);
    case undefined_error_i: throw Clipper2Exception(undefined_error);
    case range_error_i: throw Clipper2Exception(range_error);
    }
#else
...
struct Point
{
    ...
    void SetZ(const int64_t z_value)
    {
        z = z_value;
    }
...
{
    double a = 0.0;
    for (typename Paths<T>::const_iterator paths_iter = paths.cbegin(); paths_iter != paths.cend();
         ++paths_iter)
    {
        a += Area<T>(*paths_iter);
    }
...
template <typename T>
inline bool GetIntersectPoint(const Point<T> &ln1a, const Point<T> &ln1b, const Point<T> &ln2a,
                              const Point<T> &ln2b, Point<T> &ip)
{
...
inline bool SegmentsIntersect(const Point64 &seg1a, const Point64 &seg1b, const Point64 &seg2a,
                              const Point64 &seg2b, bool inclusive = false)
{
    if (inclusive)
    {
...
template <typename T>
inline Point<T> GetClosestPointOnSegment(const Point<T> &offPt, const Point<T> &seg1,
                                         const Point<T> &seg2)
{
    ...
    double q =
        (static_cast<double>(offPt.x - seg1.x) * dx + static_cast<double>(offPt.y - seg1.y) * dy)
        / (Sqr(dx) + Sqr(dy));
    if (q < 0)
        q = 0;
    else if (q > 1)
