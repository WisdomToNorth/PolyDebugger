...
inline Paths64 Minkowski(const Path64 &pattern, const Path64 &path, bool isSum, bool isClosed)
{
    ...
    if (isSum)
    {
        for (const Point64 &p : path)
        {
            ...
            std::transform(pattern.cbegin(), pattern.cend(), path2.begin(),
                           [p](const Point64 &pt2) { return p + pt2; });
            tmp.push_back(path2);
        }
    }
    else
...
inline PathsD MinkowskiSum(const PathD &pattern, const PathD &path, bool isClosed,
                           int decimalPlaces = 2)
{
    int error_code = 0;
...
inline PathsD MinkowskiDiff(const PathD &pattern, const PathD &path, bool isClosed,
                            int decimalPlaces = 2)
{
    int error_code = 0;
    double scale = pow(10, decimalPlaces);
